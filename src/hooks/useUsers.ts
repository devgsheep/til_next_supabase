...
export function useUsers() {
  ...
  return useQuery({
    ...
    queryKey: ['users'],
    // 쿼리 함수 : 실제 데이터를 가져오는 함수 연결
    queryFn: fetchUsers,
    // 쿼리 개별 옵션
    staleTime: 5 * 60 * 1000, // 5분간은 호출을 막는다. 즉 fetch 유지
    gcTime: 10 * 60 * 1000, // 10분간 캐시는 유지함.
  });
}
...
export function useUser(id: number) {
  ...
  return useQuery({
    ...
    // 사용자 ID가 null, undefined, 0 보다 작으면 실행하지 않도록
...
export function useUserWithPosts() {
  ...
  const postsQueries = useQuery({
    ...
    queryFn: async () => {
      ...
      // 상위에서 if문 등의 조건을 이용하면 정확히 자료가 있다는
      // 타입 좁히기 또는 타입가드가 적용됨
    ...
    // enabled: !!usersQuery.data && usersQuery.isSuccess,
