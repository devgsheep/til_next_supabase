...
export function useCreatePost() {
  // 꼭 알아두자
  // 아래구문은 React Query의 데이터 저장소에 접근하기 위한 훅
  ...
  return useMutation({
    // 뮤테이션 함수 : API를 이용한 새 게시글 생성 함수 연결
    mutationFn: createPost,
    // 성공시 실행되는 함수
    onSuccess: newPost => {
      // 게시글 목록 쿼리들을 무효화해서 최신 데이터를 다시 가져오도록 함.
      // 아래 구문은 특정 쿼리 키의 캐시를 무효화 함.
      // React Query가 자동으로 최신 데이터를 다시 가져오게 하는 함수
      // 지금 캐시에 저장된 posts가 오래 되었으니, 다시 서버에서 가져와
...
export function useUpdatePost() {
  ...
  return useMutation({
    // 뮤테이션 함수 : API를 이용한 게시글 업데이트 함수 연결
    // Partial 제네릭은 모든 객체 속성을 Optional로 변환 즉, ?를 모두 붙여줌
...
export function useDeletePost() {
  ...
  return useMutation({
    ...
    // 성공시
    // 아래도 기억을 합시다.
    // 첫번째 매개변수 _ 의 의미는 mutation의 결과를 말함.
    // _ 의 코딩상 의미는 사용하지 않는 변수
    // deletePost 함수 API는 결과를 리턴하는 것이 없다.
    // 사용하지 않는 리턴 결과임을 표현하기 위해서 _를 사용함

    // 첫번째 매개변수 : _ 결과값,
    // 두번째 매개변수 deletedId는 deletePost(매개변수)에 전달한 매개변수를 참조함
    // deleteMutation.mutate(123)
...
// 게시글과 댓글을 함께 가져오는 훅
export function usePostWithComment(userId?: number) {
  // 먼저 게시글 목록을 가져옴
