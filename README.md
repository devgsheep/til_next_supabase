...
### 1.3. 실제 데이터 Props 로 받아서 전달해 보자.

- `/src/components/post/PostItem.tsx` Props 전달
...
- 추후 본인이 추가적인 기능을 업데이트 시 아래 코드에서 `postEditorModalStroe` 검색으로 확인.
...
```ts
import { UseMuationCallback } from '@/types/types';
import { useMutation } from '@tanstack/react-query';
import { updatePost } from '@/apis/post';

export function useUpdatePost(callback?: UseMuationCallback) {
  return useMutation({
...
const handleSavePost = () => {
  ...
  if (postEditorModalStore.type === 'CREATE') {
    ...
    updatePost({ id: postEditorModalStore.postId, content });
  }
};
```
