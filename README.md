...
```tsx
import { CreatePostButton } from '@/components/post/GreatePostButton';
...
## 5. zustand로 modal의 상태를 전역 관리하기

### 5.1. store 만들기

- `/src/stores/postEditorModal.ts 파일` 생성
...
- 화면에 출력을 시키려면 누군가의 `자식 컴포넌트` 여야한다.
