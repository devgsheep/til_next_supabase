...
- Auth Providers > email > `Enable Email provider` 만 활성화
...
## 5. React Query의 `Mutation`의 이해

- 데이터 등록, 수정, 삭제 요청관리하기

### 5.1. 백엔드 서버에 데이터 추가 요청 보낼때 비동기 함수 필요

- `/src/apis 폴더 생성`
- 참조 : 경로 주의하셔야 합니다.
- 참조 : 절대로 `/src/app/api` 폴더에 작성하면 Next가 작동됩니다.

### 5.2. 수업 참조해 보기

- `/src/apis/create-todo.ts` : 할일 생성 API 함수

```ts
// Todo를 등록하는 함수 : API 즉, 백엔드 연동용 함수
...
- 백엔드 연동 `API가 완성`되면 이제 `관리 해줄 Mutation`을 만든다.

### 5.3. 왜 Mutation을 쓰는가?

- 현재 글을 등록하는데
- 현재 등록중인 상태를 `isPending 개발자가 코딩으로 관리한다.`
- 현재 글이 등록 완료되었는지 `response가 true면 개발자가 코딩으로 관리한다.`
- 현재 글이 등록 실패했는지 `error 처리 개발자가 코딩으로 관리한다.`
- 현재 글이 등록 오류가 발생시 `error 개발자가 코딩으로 관리한다.`
- 각각의 상황에 맞는 함수들 (이벤트에 따라서 실행할 콜백함수 등)을 직접 작성함.

### 5.4. Mutation 적용

- api 함수를 만들고 나면 mutation을 생성해 준다.
- `/src/hooks 폴더` 만들기
- `/src/hooks/mutations 폴더` 만들기
