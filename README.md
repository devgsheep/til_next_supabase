...
- /auth/callback?next=`비밀번호수정될 라우터 경로`
- env : `NEXT_PUBLIC_APP_URL=http://localhost:3000`

```ts
// supabase의 비밀번호 찾기 이메일을 전송함.
export async function requestPasswordRestEmail({ email }: { email: string }) {
...
- Supabase 의 Auth 인증 처리해주는 역할
- `/src/app/auth 폴더` 생성
- `/src/app/auth/callback 폴더` 생성

## 4.1. page.tsx 말고 `route.ts` 생성에 대한 이해

- page.tsx 는 화면의 UI 출력
- Next.js 에서는 route만 처리하는 파일명이 별도로 존재함.
- route.ts는 UI를 리턴하지 않음.
- route.ts는 Next.js의 App Router 처리를 진행함.
- route.ts는 HTTP 메서드를 처리함.
- route.ts는 NextRequest, NextResponse를 처리함.
- route.ts는 오로지 서버 컴포넌트에만 작성 가능.
- Supabase 는 서버라서 웹브라우저에 접근이 불가하다.
- 즉, 웹부러우저에 보관해주어야 할 Supabase의 쿠키를 생성못함
...
## 5. Supabase의 createClient 종류

- /src/lib/supabase/client.ts : 클라이언트 컴포넌트에서 활용(웹브라우저용)
- /src/lib/supabase/server.ts : 서버 컴포넌트에서 활용(서버용)
- /src/lib/supabase/middleware.ts : Next.js와 연동하는 경우 활용, 동기

## 6. supabase의 middleware.ts

- 화면에 페이지 즉, app/layout.tsx 를 그려내기 전 사전에 처리가 필요
- `/src/lib/supabase/middleware.ts` 리턴값 업데이트
- 쿠키 관리, Next.js의 middleware에 활용
...
- 예) layout.tsx, page.tsx, loading.tsx ...
...
- `/src/app/forget-password/page.tsx` 업데이트
...
- `/src/hooks/mutations/useUpdatePassword.ts 파일` 생성
