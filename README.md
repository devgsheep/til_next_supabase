...
- 수업인데 Supabase 계정이 2개라서 오류가 나면 아래부터 진행
...
- Supabase 는 `React 라면 localstorage에 로그인 정보` 보관
- localstorage는 웹브라우저 종료해도 남아있어요.
- localstorage는 새로고침해도 남아있어요
- localstorage는 유효기한이 없어요.

- Supabase 는 `Next 라면 cookie에 로그인 정보` 보관
...
export const useSession = () => {
  // Selector 함수는 Store 원하는 것을 선택해서 리턴한다.
...
- Supabase 인증은 자동으로 cookie에 보관됩니다.
- local Storage에 보관하지 않아도 웹브라우로 가지고 있습니다.

## 4. 활용하기

### 4.1. 전역에서 활용하기 위해서 /src/app/layout.tsx 입니다.

### 4.2. 그런데, "use client"를 layout.tsx에 적용은 곤란함.

### 4.3. 별도의 컴포넌트를 만들어서 layout.tsx에 배치 권장

- `/src/components/providers 폴더` 생성
- `/src/components/providers/SessionProvider.tsx` 생성
