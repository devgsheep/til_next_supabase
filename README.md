...
- bil : `text`, `Empty`, `Not Null`
- avatar_url : `text`
- role: `text`, "MEMBER", `Not Null`
- 반드시 SAVE 버튼을 클릭하자.

## 3. Type Script 데이터형 출력
...
- 일반적으로 회원가입 후 자동으로 DB에서 트리거를 통해 회원정보 입력이 기본
- `트리거`는 JS의 `이벤트 핸들러`라고 생각하면 됩니다.
- 어떤 이벤트가 발생하면 이후 자동으로 실행되는 함수를 말함
- DB에 사용자가 추가되면 `새로운 profiles에 데이터를 입력하라`는 것
...
- 리액트에서 처리를 하여도 됩니다.
- 최대한 안전하게, 그리고 대응이 수월하도록 React에서 처리함.

## 6. 애플리케이션에서 처리하는 과정

- 회원가입 완료
- 완료 후 나의 프로필이 있는지 조회 진행 (select)
- 조회의 결과가 없다면
- 프로필을 생성하는 단계 요청
- 데이터 조회의 과정은 useQuery로 진행
- 데이터 입력의 과정은 useMutation으로 진행
...
- 불필요한 데이터 요청이 들어오면 캐시데이터를 즉시 리턴함.

## 2. 최신 데이터를 반영하여야 하는 경우
...
- 1번 단계
- api 요청시 `데이터가 불러들여지는 중의 상태`
- 캐시에 보관안됨.
- 완료 되면 `Fresh 상태`로 변경됨.
...
- staleTime이 `글로벌로는 0` 으로 세팅됨.
- staleTime이 `개별`로 세팅가능.
...
- `unMount 상태일 때`
- gcTime이 `글로벌로는 30분` 으로 세팅됨.
- gcTime이 `개별`로 세팅가능.
- gcTime이 지나면 deleted 상태로 됨
...
- 캐시 데이터를 사용하는 컴포넌트가 화면에 보일 때

### 4.2. WindowFocus

- 사용자가 다른 탭에 갔다가 다시 원래의 탭을 선택할 때
